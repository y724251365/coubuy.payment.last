...
export class HistoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._init.getCard().then(res2 => {
      ...
      // 获取用户购物车历史订单
      this._init.getOrderHistory(this.cardNum, this.i).then(res3 => {
        this.historyProduct = res3.json().result.data;
        this.count = res3.json().result.count;
      ...
      if (getScrollTop() + getClientHeight() >= getScrollHeight()) {
        ...
        if (this.historyProduct) {
          if (this.historyProduct.length < this.count) {
            ...
            // console.log(this.count);
            this.i += 1;
            this._init.getOrderHistory(this.cardNum, this.i)
              .then(res => {
            ...
            if (this.historyProduct.length == 0) {
              this.isShow = false;
            } else {
              this.isShow = true;
            }
